...
from app.schemas.recommendation import OutfitResponse, ProductResponse, ScoreBreakdownResponse


def ranked_product_to_response(item: RankedProduct | None) -> ProductResponse | None:
    ...
    return ProductResponse(
        ...
        score=item.score,
        breakdown=ScoreBreakdownResponse(**item.breakdown.__dict__),
    )
...
def outfit_to_response(outfit: Outfit) -> OutfitResponse:
    ...
    return OutfitResponse(
        ...
        accessories=[item for item in (ranked_product_to_response(accessory) for accessory in outfit.accessories) if item],
        compatibility_score=outfit.compatibility_score,
        reasoning=outfit.reasoning,
        generated_image_path=outfit.generated_image_path,
    )
